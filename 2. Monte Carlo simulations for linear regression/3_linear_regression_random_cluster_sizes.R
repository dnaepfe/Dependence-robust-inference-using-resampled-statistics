...
#helper functions
random_partition <- function(n,min_val,max_val){
  sizes <- c()
  remaining <- n
  while (remaining > 0){
    if( n - sum(sizes) <= max_val){
      sizes[length(sizes)+1] <- n - sum(sizes)
      break
    }
    s <- sample(min_val:max_val,1)
    sizes <- c(sizes,s)
...
#simulation
for(s in seq_along(bound_num)){
  lower <- bounds[s,1]
  upper <- bounds[s,2]
  
  cluster_sizes <- random_partition(n,lower,upper)
  num_clusters <- length(cluster_sizes)
  cluster_id    <- rep(1:num_clusters, times = cluster_sizes)
  
  cat("Simulation for lower=", lower, "and upper=", upper, "\n")
  ...
  summary3[s, "Type1_b0_t"] <- mean(results$Type1_b0_t)
  summary3[s, "Type1_b1_t"] <- mean(results$Type1_b1_t)
  summary3[s, "Type1_b0_m"] <- mean(results$Type1_b0_m)
  summary3[s, "Type1_b1_m"] <- mean(results$Type1_b1_m)
  summary3[s, "Type1_b0_u"] <- mean(results$Type1_b0_u)
  summary3[s, "Type1_b1_u"] <- mean(results$Type1_b1_u)
  summary3[s, "Type1_b0_s"] <- mean(results$Type1_b0_s)
  summary3[s, "Type1_b1_s"] <- mean(results$Type1_b1_s)
  
  summary3[s, "Power_b0_t"] <- mean(results$Power_b0_t)
  summary3[s, "Power_b1_t"] <- mean(results$Power_b1_t)
  summary3[s, "Power_b0_m"] <- mean(results$Power_b0_m)
  summary3[s, "Power_b1_m"] <- mean(results$Power_b1_m)
  summary3[s, "Power_b0_u"] <- mean(results$Power_b0_u)
  summary3[s, "Power_b1_u"] <- mean(results$Power_b1_u)
  summary3[s, "Power_b0_s"] <- mean(results$Power_b0_s)
  summary3[s, "Power_b1_s"] <- mean(results$Power_b1_s)
...
long4 <- summary3 |>
  mutate(bound_label = factor(bound,
                              levels = 1:3,
                              labels = c("[4, 8]", "[5, 15]", "[10, 30]"))) |>
  pivot_longer(
    cols = -c(bound, bound_label),
    names_to  = "metric",
    values_to = "rate"
  ) |>
  mutate(
...
base_theme4 <- list(
  ...
  labs(x = "Cluster size range [min, max]", y = "Rejection rate"),
...
p_size4 <- long4 |>
  ...
  ggplot(aes(x = bound_label, y = rate,
...
ggsave("size_plot4.png",  p_size4,  width = 6, height = 6, dpi = 150)
ggsave("power_plot4.png", p_power4, width = 6, height = 6, dpi = 150)
